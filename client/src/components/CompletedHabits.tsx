...
export const CompletedHabits = ({ contract }: CompletedHabitsProps) => {
  ...
  const { data: allHabits, refetch: refetchHabits } = useReadContract({
    contract,
    method:
      "function getAllHabits(address _user) view returns ((string title, address user, uint256 startTime, uint256 durationDays, uint256 dailyStake, uint256 checkIns, uint256 streak, bool active, uint256 lastCheckIn, uint256 completedAt)[] memory)",
  ...
  useEffect(() => {
    ...
    setCompletedHabits(
      (allHabits || [])
        .filter(
          (habit: Habit) => habit.checkIns >= Number(habit.durationDays) * 0.8
        )
        .map((habit: Habit) => {
